...
/**
 * Child Layout Component (SPA Form).
 * Wraps the Child Dashboard views with a glassmorphic sidebar.
 */
export function ChildLayout({ children, activeTab, onTabChange }: { children: React.ReactNode, activeTab: string, onTabChange: (tab: string) => void }) {
    ...
    const navItems = [
        { id: "dashboard", label: "My Tasks", icon: Home },
            ...
            >
                <div className="h-full bg-white/10 backdrop-blur-glass border border-white/20 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] flex flex-col p-5 gap-6">
                    ...
                    <div className="flex items-center gap-3 pb-4 border-b border-white/10">
                        <MascotAvatar state="dopamine" className="w-10 h-10" />
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-typography truncate">
                                {profile?.display_name || "Explorer"}
                            </p>
                            <p className="text-[10px] font-medium text-dopamine-yellow uppercase tracking-widest">
                                ★ Explorer Mode
                            </p>
                        </div>
                    </div>
                                ...
                                <button key={item.id} onClick={() => onTabChange(item.id)} className="w-full text-left">
                                    <motion.div
                                        ...
                                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${isActive
                                            ? "bg-dopamine-yellow/20 border border-dopamine-yellow/30 text-dopamine-yellow shadow-[0_0_20px_rgba(255,215,87,0.15)]"
                                            : "text-typography/70 hover:bg-white/10 hover:text-typography"
                                            }`}
                                    >
                                        <Icon size={18} className={isActive ? "text-dopamine-yellow" : ""} />
                                        <span className="text-sm font-semibold">{item.label}</span>
                                        {isActive && (
                                            <motion.div
                                                layoutId="child-nav-indicator"
                                                className="ml-auto w-1.5 h-1.5 rounded-full bg-dopamine-yellow"
